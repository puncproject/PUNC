...
# Get the mesh
mesh, bnd = load_mesh("../mesh/3D/laframboise_sphere_in_cube_res1")
...
if os.path.isfile('stop'):
    os.remove('stop')
    pop.init_new_specie('electron', ext_bnd, v_thermal=vthe,
                        num_per_cell=npc, empty=True)
    pop.init_new_specie('proton',   ext_bnd, v_thermal=vthi,
                        num_per_cell=npc, empty=True)
...
for n in range(nstart,N):
    ...
    potential = objects[0]._potential / Vnorm
    ...
    current_measured = ((objects[0].charge - old_charge) / dt) / Inorm
