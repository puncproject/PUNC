...
for vsource in vsources:
    ...
    alphaid = int_bnd_ids[alpha]
    betaid  = int_bnd_ids[beta]

    area_pair = areas[alpha]+areas[beta]
    ds_pair = dss(alphaid)+dss(betaid) 

    a += inner(c[i], (1./areas[beta])*v)*dss(betaid) -\
         inner(c[i], (1./areas[alpha] )*v)*dss(alphaid)  +\
         inner(d[i], (1./areas[beta])*u)*dss(betaid) -\
         inner(d[i], (1./areas[alpha] )*u)*dss(alphaid)
    L += inner((bias/area_pair), d[i]) * ds_pair
